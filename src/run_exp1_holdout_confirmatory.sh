...
#
# Q1 step 2 (A.6): confirmatory/exploratory analysis of the regime L hold-out
# datasets (src/experiments/exp1_holdout_confirmatory.py) ->
# results/tables/[<mode>/]exp1_holdout_confirmatory.csv/.tex. Requires
# dataset_properties.py, exp1_dr_benchmark.py (core+holdout) and
# screen_regime_candidates.py to have already run in the SAME mode (exp6_alpha_curves.py
# optional, only for F_A6 - otherwise a warning in the log, the rest of the table
# is still generated). Parameter: quick|full|smoke (default full).
...
"$PYTHON" -m src.experiments.exp1_holdout_confirmatory --"$MODE"
