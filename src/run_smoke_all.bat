...
REM
REM Runs --smoke for all five experiments (E1-E5) sequentially - every
REM dataset x method combination (1 seed, minimal budget) for a fast
REM check of all code paths. Results go into results/data/smoke/ (separate from
REM the quick/full data). Calls src/run_all_experiments.bat smoke.
setlocal
cd /d "%~dp0"
cd ..
...
call src\run_all_experiments.bat smoke
