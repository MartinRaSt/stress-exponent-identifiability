...
REM
REM Runs exp11 (convergence-vs-local-minimum diagnostic for the exp10
REM both_gaps_nonneg==False rows, documentation/2026-09-17_zadani_exp11_konvergence.md,
REM src/experiments/exp11_convergence_check.py) ->
REM results/data/[<mode>/]exp11_convergence_check_results.csv + _DONE.txt.
REM Requires COMPLETED exp6_alpha_curves AND exp10_identifiability_check FULL
REM runs (read regardless of the mode given here - see the module docstring;
REM fail-loud on missing inputs). Parameter: quick|full|smoke (default full).
setlocal
cd /d "%~dp0.."
...
"venv\python.exe" -m src.experiments.exp11_convergence_check --%MODE%
