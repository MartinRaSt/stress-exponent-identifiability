...
#
# Runs exp10 (numerical identifiability check of the stress exponent,
# reserse/2026-09-17_zostreni_propozice2.md section 10,
# src/experiments/exp10_identifiability_check.py) ->
# results/data/[<mode>/]exp10_identifiability_check_results.csv + _DONE.txt.
# NO new DR computation - only loads .npy embeddings and the original data.
# Requires COMPLETED exp6_alpha_curves AND exp8_prop2_check runs in the SAME
# mode (otherwise fail-loud on missing inputs). Parameter: quick|full|smoke (default full).
...
"$PYTHON" -m src.experiments.exp10_identifiability_check --"$MODE"
