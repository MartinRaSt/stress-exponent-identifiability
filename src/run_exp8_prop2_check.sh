...
#
# Q1-step1: runs exp8 (post-hoc empirical test of Proposition 2 over the finished
# exp6_alpha_curves embeddings, src/experiments/exp8_prop2_check.py) ->
# results/data/[<mode>/]exp8_prop2_check_results.csv + _DONE.txt. NO new
# DR computation - just loads .npy and the original data, runs in minutes.
# Requires a completed exp6_alpha_curves run in the same mode (otherwise
# fail-loud on missing embeddings). Parameter: quick|full|smoke (default full).
...
"$PYTHON" -m src.experiments.exp8_prop2_check --"$MODE"
