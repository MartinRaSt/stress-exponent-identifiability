...
REM
REM Runs src/main.py: aggregation of all experiment results, Friedman/Nemenyi
REM statistics, tables (results/tables/*.csv + *.tex) and all figures
REM (src/figures/*.py) + number macros (export_numbers). Parameters:
REM   run_main.bat [quick|full|smoke] [--no-figures | --figures-only]
REM full: results/tables/, results/figures/, clanek/generated/numbers.tex;
REM quick|smoke: EXCLUSIVELY results/tables/<mode>/, results/figures/<mode>/,
REM results/tables/<mode>/numbers.tex (author's rule from 2026-09-13).
REM --no-figures: only statistics+tables+macros (minutes); --figures-only: only figures.
setlocal
cd /d "%~dp0.."
...
REM additional parameters (--no-figures | --figures-only) are passed through
set EXTRA=%2 %3
"venv\python.exe" -m src.main --%MODE% %EXTRA%
