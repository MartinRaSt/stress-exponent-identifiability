...
#
# K1: computes properties of the E1+E3 input datasets
# (src/experiments/dataset_properties.py) -> results/data/[<mode>/]
# dataset_properties.csv. Parameter: quick|full|smoke (default full).
...
# --check-correlation requires exp1_dr_benchmark to have already run in the same
# mode (results/data/[<mode>/]exp1_dr_benchmark_results.csv) - in full mode
# it additionally computes and prints Spearman(nn_ratio_k1, best alpha
# from {0,1,2}); in quick/smoke mode it is skipped (data may not exist).
if [ "$MODE" = "full" ]; then
    "$PYTHON" -m src.experiments.dataset_properties --"$MODE" --check-correlation
else
    "$PYTHON" -m src.experiments.dataset_properties --"$MODE"
fi
