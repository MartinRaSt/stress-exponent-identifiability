...
#
# E15 statistics: paired reference_method vs. report.main_methods comparison
# (McNemar + Wilcoxon, Holm-corrected) over the already finished
# exp15_discovery_task_results.csv -> results/tables/[<mode>/]
# exp15_discovery_task_summary.csv + exp15_discovery_task_pairwise.csv.
# Fast run (a few seconds). Requires run_exp15_discovery_task.sh to have
# already completed in the same mode. Parameter: quick|full|smoke (default full).
...
"$PYTHON" -m src.experiments.exp15_discovery_task_stats --"$MODE"
