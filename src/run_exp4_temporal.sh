...
#
# Runs E4 (temporal alpha-Sammon, src/experiments/exp4_temporal.py).
# Parameter: quick|full|smoke (default full). Resumable, incl. the permutation
# test (exp4_stats.csv) and trajectories (exp4_trajectories.csv).
#
# 2026-09-16: E4 runs IN PARALLEL over (dataset, method, seed) on
# parallel.n_workers workers (previously sequential). Estimate of the full grid from
# measured times: 4.36 h sequential -> approx. 30 min on 12 workers
# (src/run_estimate_runtime.sh exp4_temporal).
# The BLAS thread setting below is therefore REQUIRED, not just recommended.
