...
REM
REM Runs E2 (SGD convergence, scaling with n, approximation error of the sparse
REM model, src/experiments/exp2_solver_scaling.py). Parameter: quick|full|smoke
REM (default full). Resumable, three independently checkpointed CSV outputs.
setlocal
cd /d "%~dp0.."
...
REM processors out of 32 available, see config.yaml parallel.n_workers). The sequential
REM part of E2 (large n) additionally uses parallel.blas_threads_sequential via
REM threadpoolctl inside run_experiment_grid, see src/experiments/exp_common.py.
...
"venv\python.exe" -m src.experiments.exp2_solver_scaling --%MODE%
