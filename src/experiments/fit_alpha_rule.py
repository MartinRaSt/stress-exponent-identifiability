...
def _write_booktabs_tex(df: pd.DataFrame, out_path: Path, caption: str, label: str, col_formats: dict[str, str] | None = None) -> None:
    """`col_formats`: an optional dict column-name -> a Python format-spec
    without a leading '%' for float columns (e.g. '.2f'); a missing column
    uses the default '.4f'. Used for more readable (fewer decimal places)
    split LOO tables - see `_LOO_CORE_FORMATS`/`_LOO_VARIANTS_FORMATS` in `main()`."""
    cols = list(df.columns)
    fmts = col_formats or {}
    lines = [
        "% auto-generated by src/experiments/fit_alpha_rule.py - do not edit by hand",
        "\\begin{table}[htbp]", "\\centering", f"\\caption{{{caption}}}", f"\\label{{{label}}}",
        f"\\begin{{tabular}}{{{'l' * len(cols)}}}", "\\toprule", " & ".join(cols) + " \\\\", "\\midrule",
    ]
    for _, row in df.iterrows():
        cells = [f"{v:{fmts.get(c, '.4f')}}" if isinstance(v, float) else str(v) for c, v in zip(cols, row)]
...
def main() -> None:
    ...
    _write_booktabs_tex(
        ...
        "Leave-one-dataset-out validation of the alpha prediction rule: per-dataset $\\rho_{NN}$, $\\alpha^\\ast$, and AUC$_{RNX}$ at $\\alpha_{\\text{pred}}$ vs. fixed $\\alpha\\in\\{0,1\\}$",
        "tab:alpha_pred_loo", col_formats=_LOO_CORE_FORMATS,
    )
    ...
    _write_booktabs_tex(
        ...
        "Leave-one-dataset-out validation of the alpha prediction rule: candidate-variant comparison (log-linear vs. two-threshold) and K6 timing",
        "tab:alpha_pred_loo_variants", col_formats=_LOO_VARIANTS_FORMATS,
    )
