...
def _write_summary_table(experiment_name: str, mode: str, logger) -> Path | None:
    ...
    # One shared table writer (report_tables.write_booktabs_tex): human-readable
    # column labels from display_labels and multi-line 	head headers. The hand
    # written version here printed raw identifiers ("auc_rnx_median"), which are
    # single unbreakable words and blew the table past the supplement text width.
    write_booktabs_tex(
        summary, tables_dir / "exp7_rank_weights_summary.tex",
        caption="Rank-weighted vs. distance-weighted (alpha-Sammon) stress - median over "
                "seeds then datasets (negative supplement result)",
        label="tab:exp7_rank_weights_summary",
        comment_lines=["source: " + str(csv_path) + ", see _write_summary_table"],
    )
