...
#
# Q1 step 2 (A.7): power analysis of the confirmatory H1 test (regime L)
# (src/experiments/power_analysis_regime.py) ->
# results/tables/[<mode>/]power_analysis_regime.csv. Parameter:
# quick|full|smoke (default full) - the only difference between modes is n_boot for the
# Monte Carlo (see config_experiments.yaml power_analysis_regime); the input
# data (10 observed Delta) are ALWAYS the same (independent of other
# experiments - no dependency on a completed E1/E6).
...
"$PYTHON" -m src.experiments.power_analysis_regime --"$MODE"
