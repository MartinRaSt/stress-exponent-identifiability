...
#
# Runs exp11 (convergence-vs-local-minimum diagnostic for the exp10
# both_gaps_nonneg==False rows, documentation/2026-09-17_zadani_exp11_konvergence.md,
# src/experiments/exp11_convergence_check.py) ->
# results/data/[<mode>/]exp11_convergence_check_results.csv + _DONE.txt.
# Requires COMPLETED exp6_alpha_curves AND exp10_identifiability_check FULL
# runs (read regardless of the mode given here - see the module docstring;
# fail-loud on missing inputs). Parameter: quick|full|smoke (default full).
...
"$PYTHON" -m src.experiments.exp11_convergence_check --"$MODE"
